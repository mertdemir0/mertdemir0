...
def simple_request(func_name, query, variables):
    ...
    """
    request = resilient_post('https://api.github.com/graphql', {'query': query, 'variables': variables}, HEADERS)
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    root = tree.getroot()
    justify_format(root, 'age_data', new_text=age_data, length=50)
...
if __name__ == '__main__':
    """
    Mert Demir (mertdemir0), 2022-2025
    """
    print('Calculation times:')
    # define global variable for owner ID and calculate user's creation date
    # e.g {'id': 'MDQ6VXNlcjU3MzMxMTM0'} and 2019-11-03T21:15:07Z for username ''
    user_data, user_time = perf_counter(user_getter, USER_NAME)
    OWNER_ID, acc_date = user_data
    formatter('account data', user_time)
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(1995, 2, 23))
    ...
    if OWNER_ID == {'id': 'MDQ6VXNlcjIyNzMzNzM1'}: # only calculate for user mertdemir0
